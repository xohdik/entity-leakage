"""CodeBERT on (bug report, patch) -> correctness, given a split file.

Usage:
  HF_ENDPOINT=https://hf-mirror.com python scripts/train_quatrain.py \
    --split data/patches/qt_split_random.json --seed 42 --out runs/qt_rand_s42
Saves per-example predictions to <out>/preds.json.
"""
...
class DS(Dataset):
    ...
    def __getitem__(self, i):
        r = self.rows[i]
        enc = self.tok(r["report"], r["patch"], truncation="longest_first",
                       max_length=384, padding="max_length", return_tensors="pt")
...
def main():
    ...
    try:
        contam = set(json.load(open("data/patches/qt_contaminated_test.json")))
