...
for f in sorted(glob.glob("runs/*/result.json")):
    ...
    bench = "Devign" if name.startswith(("pub","clu")) else "Patch"
    proto = ("shipped" if name.startswith("pub") else "cluster-safe") if bench=="Devign" \
            else ("patch-level" if "rand" in name else "bug-level")
    w.writerow([name, bench, proto, cfg(f"runs/{name}"), r.get("seed"),
